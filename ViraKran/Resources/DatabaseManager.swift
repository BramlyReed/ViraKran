...
final class DatabaseManager{
    ...
    func getInfoAboutCurrencies(){
        ...
        var currenciesRatesArray = Rates(GBP: 0, USD: 0, EUR: 0)
        URLSession.shared.dataTask(with: url) { data, _, _ in
            if let data = data, let parsedCurrencies = try? JSONDecoder().decode(ParsedCurrencies.self, from: data){
                currenciesRatesArray = parsedCurrencies.rates
                UserDefaults.standard.set(currenciesRatesArray.EUR, forKey: "EUR")
                UserDefaults.standard.set(currenciesRatesArray.GBP, forKey: "GBP")
                UserDefaults.standard.set(currenciesRatesArray.USD, forKey: "USD")
            }
            else{
